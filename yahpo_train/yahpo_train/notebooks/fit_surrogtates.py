...
def tune_config(fitter, name):
    ...
    # lr = 1e-4, epochs=50, deep=[512,512,256], deeper=[], dropout=0.5, wide=True,  bs=2048, mixup = False, 
    def objective(trial):
        lr = trial.suggest_float("lr", 1e-5, 1e-3, log=True)
        deep_u = trial.suggest_categorical("deep_u", [0,7,8,9,10])
        deep_n = trial.suggest_categorical("deep_n", [0,1,2,3])
        deep_s = trial.suggest_categorical("deep_s", ["square", "cone"])
        deeper_u = trial.suggest_categorical("deeper_u",[0,7,8,9,10])
        ...
        dropout = trial.suggest_categorical("dropout", [0., 0.3])
        ...
        if use_deeper:
            ...
            deeper = []
        if deep_u == 0 and not use_deeper:
            deep = [512,256,128]
        l = fitter(lr = lr, deep = deep, deeper = deeper, wide = wide, cbs=cbs, mixup=mixup, use_bn=use_bn, dropout=dropout, log_wandb=False)
...
def fit_rbv2_rpart(key = 'rbv2_rpart', **kwargs):
    ...
    [tfms.update({k:partial(ContTransformerRange)}) for k in ["timetrain", "timepredict"]]
